...
namespace _PhpScoper9a3678ae6a12\Carbon_Fields\REST_API;
...
class Router
{
    ...
    protected function get_all_field_values($container_type, $object_id = null)
    {
        ...
        foreach ($fields as $field) {
            ...
            if (apply_filters('carbon_fields_rest_api_return_attachments_as_urls', \false, $value, $field, $object_id)) {
                $attachments_class = ["_PhpScoper9a3678ae6a12\\Carbon_Fields\\Field\\Media_Gallery_Field", "_PhpScoper9a3678ae6a12\\Carbon_Fields\\Field\\File_Field", "_PhpScoper9a3678ae6a12\\Carbon_Fields\\Field\\Image_Field"];
    ...
    protected function set_options($request)
    {
        ...
        if (empty($options)) {
            return new \_PhpScoper9a3678ae6a12\WP_REST_Response(__('No option names provided', 'carbon-fields'));
        }
        foreach ($options as $key => $value) {
            try {
                Helper::set_value(null, 'Theme_Options', '', $key, $value);
            } catch (\Exception $e) {
                return new \_PhpScoper9a3678ae6a12\WP_REST_Response(wp_strip_all_tags($e->getMessage()));
            }
        }
        return new \_PhpScoper9a3678ae6a12\WP_REST_Response(__('Theme Options updated.', 'carbon-fields'), 200);
    }
    /**
    ...
    public function block_renderer_permission($request)
    {
        ...
        if (0 < $post_id) {
            ...
            if (!$post || !current_user_can('edit_post', $post->ID)) {
                return new \_PhpScoper9a3678ae6a12\WP_Error('block_cannot_read', __('Sorry, you are not allowed to read blocks of this post.', 'carbon-fields'), array('status' => rest_authorization_required_code()));
            }
        } else {
            if (!current_user_can('edit_posts')) {
                return new \_PhpScoper9a3678ae6a12\WP_Error('block_cannot_read', __('Sorry, you are not allowed to read blocks as this user.', 'carbon-fields'), array('status' => rest_authorization_required_code()));
            }
        }
        return \true;
    }
    ...
    public function block_renderer($request)
    {
        ...
        $registry = \_PhpScoper9a3678ae6a12\WP_Block_Type_Registry::get_instance();
        $block = $registry->get_registered($request['name']);
        if (null === $block) {
            return new \_PhpScoper9a3678ae6a12\WP_Error('block_invalid', __('Invalid block.'), array('status' => 404));
        }
